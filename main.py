...
def level1():
    ...
    if number in range(1, 32):
        ...
        solutions_svg([all_solutions[0]], filename='first_solution.svg',
                      date_number=number, date_position=(row, column),
                      month_name='', month_position=(0, 0), day_name='', day_position=(0, 0),
                      level=1, columns=7, colour=COLOURS.get)
        with open("first_solution.svg", "r") as file:
            svg_content = file.read()
        st.image(svg_content, use_container_width=True)
    else:
        st.write("Enter a valid number between 1-31")
...
def level3():
    ...
    if number2 in range(1, 32) and month_input1 in months and day_input in days:
        month_map = {month: i + 1 for i, month in enumerate(months)}
        day_map = {day: (6 + i // 3, 3 + i % 3) for i, day in enumerate(days)}
        ...
        GRID = Grid((8, 7), holes=[(0, 6), (1, 6), (7, 0), (7, 1), (7, 2), 
                                   (7, 3), (row, column), (m_row, m_column), day_number2])
        ...
        solutions_svg(all_solutions, filename='first_solution.svg',
                      date_number=number2, date_position=(row, column),
                      month_name=month_input1, month_position=(m_row, m_column),
                      day_name=day_input, day_position=day_number2, level=3, columns=7, colour=COLOURS.get)
        with open("first_solution.svg", "r") as file:
            svg_content = file.read()
        st.image(svg_content, use_column_width=True)
    else:
        st.write("Enter valid information")
