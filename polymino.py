...
def solutions_svg(solutions, filename,date_number,date_position,level,month_name,month_position,day_name,day_position,columns=1, size=50, padding=5,
                  colour=lambda _: "white", stroke_colour="black",
                  stroke_width=5, empty=' '):
    """Format polyomino tilings as an SVG image.
    ...
    for i, solution in enumerate(solutions):
        ...
        for polymino in solution.polyminoes:
            ...
            for i, j in polymino.coord:
                ...
                piece.add(drawing.rect((j * size + oj, i * size + oi),
                                       (size, size)))
        if level == 1:
            row, col = date_position
            # Calculate the x and y coordinates for the text
            x_coord = col * size + oj + size / 2  # Center text horizontally in the cell
            y_coord = row * size + oi + size / 2  # Center text vertically in the cell
            if date_number in (33,34,35):
        ...
        if level == 2:
            ...
            date_x_coord = date_col * size + oj + size / 2  # Center text horizontally
            date_y_coord = date_row * size + oi + size / 2  # Center text vertically
            
            drawing.add(drawing.text(
                str(date_number),
                insert=(date_x_coord, date_y_coord),
                fill='white',
                font_size=size // 2,  # Adjust font size
                text_anchor="middle",
            ...
            month_x_coord = month_col * size + oj + size / 2  # Center text horizontally
            month_y_coord = month_row * size + oi + size / 2  # Center text vertically

            drawing.add(drawing.text(
                month_name,
                insert=(month_x_coord, month_y_coord),
                fill='white',
                font_size=size // 3,  # Smaller font for the month
                text_anchor="middle",
                alignment_baseline="middle"
            ))
        if level == 3:
            ...
            day_x_coord = day_col * size + oj + size / 2  # Center text horizontally
            day_y_coord = day_row * size + oi + size / 2  # Center text vertically
